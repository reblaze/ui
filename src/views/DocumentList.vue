  ...
  <div class="card">
    <div class="card-content">
          ...
          <div class="field is-grouped">
            <div class="control" v-if="branchNames.length">
              <div class="select is-small">
                <!--TODO: add to input below  data-qa="policies-switch-branch" @change="switchBranch()" -->
                <select
      ...
      <table
        class="
          table
          is-bordered is-fullwidth is-size-7
          document-list-table
          is-hoverable
          vectors-table">
        <thead>
          <tr>
              ...
              <div v-if="col.isSortable">
                <div class="arrow-wrapper">
                  <span
                    ...
                    :class="{
                      active: sortField === col.fieldName && sortDir === 'asc', }"/>
                </div>
                <div class="arrow-wrapper">
                  <span
                    class="arrow arrow-desc"
                    :class="{
                      active: sortField === col.fieldName && sortDir === 'desc', }"/>
                </div>
              </div>
              {{ col.columnTitle }}
            </th>
            <th class="column-header is-2">
              <div class="field is-grouped is-grouped-centered">
                ...
                <p class="control">
                  <button
                    class="button is-size-7 filter-toggle"
                    :class="{ 'is-active': filtervisible }"
                    title="Filter columns and rows"
          ...
          <tr class="search-row" v-if="filtervisible">
            <th
              ...
              <div v-if="col.isSearchable">
                <input
                  class="input is-small filter-input search-input-vectors-score"
        ...
        <tbody>
          <tr
            v-for="row in getSlicedDataArrayDisplay(rows, currentPage)"
            :key="row.id">
...
/*----Responsible for the unmarking the even rows, should to check------*/
.vctors-table.table tbody tr:not(.is-selected):nth-child(even) {
