<template>
  <div>
    <div class="has-text-left has-text-weight-bold pb-3">
    ...
    <table class="table connected-security-policies-table">
      <thead>
        <tr>
          ...
          <th class="is-size-7 width-300px">
            Domain Match
          </th>
          <th class="is-size-7 width-300px">
            Entry Match
          ...
          <th class="is-size-7 width-70px has-text-centered">
            <a
              v-if="!newSecurityPolicyConnectionOpened"
      ...
      <tbody>
          ...
          <template v-if="newSecurityPolicyConnections.length > 0">
            <td>
              <div class="select is-small">
                <select
                  v-model="newSecurityPolicyConnectionDataMapId"
                ...
                >
                  <option
                    v-for="map in newSecurityPolicyConnections"
                    :key="map.id"
                    :value="map.id"
                  >
                    {{ map.name }}
            ...
            <td>
              <div class="select is-small">
                <select
                  v-model="newSecurityPolicyConnectionData.entryIndex"
                  class="new-connection-entry-index"
                  data-qa="site-path-dropdown"
                  title="Type"
                >
                  <option
                    v-for="(mapEntry, index) in newSecurityPolicyConnectionEntries"
                    :key="mapEntry.match"
                    :value="index"
                  >
                    {{ mapEntry.match }}
            ...
            <td class="has-text-centered">
              ...
              >
                <span class="icon is-small"><i class="fas fa-plus fa-xs" /></span>
              </button>
            </td>
          </template>
          <template v-else>
        ...
        <tr
          v-for="(connection, index) in connectedSecurityPoliciesEntries"
          :key="index"
        >
          <td
            class="is-size-7 is-vcentered py-3 width-150px connected-entry-row"
            :title="connection[0]"
          >
            <a
              title="Add new"
          ...
          <td
            class="is-size-7 is-vcentered py-3 width-120px"
            :title="connection.id"
          >
            {{ connection.id }}
          </td>
          <td
            class="is-size-7 is-vcentered py-3 width-300px ellipsis"
            :title="connection.domainMatch"
          >
            {{ connection.domainMatch }}
          </td>
          <td
            class="is-size-7 is-vcentered py-3 width-300px"
          ...
          <td class="is-size-7 is-vcentered width-70px height-50px">
            ...
            <span v-show="currentEntryDeleteIndex === index">
              <a
                class="is-size-7 has-text-grey add-button confirm-remove-connection-button"
              ...
              >
                <i class="fas fa-check" /> Confirm
              </a>
              <br>
              <a
                class="is-size-7 has-text-grey cancel-remove-connection-button"
