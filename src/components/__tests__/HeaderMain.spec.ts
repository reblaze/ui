...
describe('HeaderMain.vue', () => {
  ...
  test('should render given version', async () => {
    // const defaultVersion = '0.0.0'
    // const wantedVersion = '1.2.3.4'
    const wrapper = shallowMount(HeaderMain)
    wrapper.vm.$forceUpdate()
    // await wrapper.setData({version: wantedVersion})
    expect(wrapper.find('div.version-box').text()).toContain(wrapper.vm.version)
  })
})
