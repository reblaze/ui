...
describe('EntriesRelationList.vue', () => {
  ...
  describe('rule prop validator', () => {
    ...
    beforeEach(() => {
      validator = (wrapper.vm as DOMWrapper).$options.props.rule.validator
    })
  ...
  describe('add entry button', () => {
    ...
    test('should add new entry from input when confirm button is clicked', async () => {
      ...
      expect((wrapper.vm as DOMWrapper).rule.sections[0].entries.length).toEqual(3)
      expect((wrapper.vm as DOMWrapper).rule.sections[0].entries[2]).toEqual(['ip', '1.2.3.4', 'annotation'])
    })
    ...
    test('should add new entry from input with general annotation when confirm button is clicked', async () => {
      ...
      await confirmAddEntryButton.trigger('click')
      expect((wrapper.vm as DOMWrapper).rule.sections[0].entries.length).toEqual(3)
      expect((wrapper.vm as DOMWrapper).rule.sections[0].entries[2]).toEqual(['ip', '1.2.3.4', 'annot'])
    })
    ...
    test('should add multiple new entries from input when confirm button is clicked', async () => {
      ...
      const confirmAddEntryButton = component.find('.confirm-add-entry-button')
      await confirmAddEntryButton.trigger('click')
      expect((wrapper.vm as DOMWrapper).rule.sections[0].entries.length).toEqual(4)
      expect((wrapper.vm as DOMWrapper).rule.sections[0].entries[2]).toEqual(['ip', '1.2.3.4', 'annotation'])
      expect((wrapper.vm as DOMWrapper).rule.sections[0].entries[3]).toEqual(['ip', '127.0.0.1', 'localhost'])
    })
    ...
    test('should add new entries from name-value input when confirm button is clicked', async () => {
      ...
      await confirmAddEntryButton.trigger('click')
      expect((wrapper.vm as DOMWrapper).rule.sections[0].entries.length).toEqual(3)
      expect((wrapper.vm as DOMWrapper).rule.sections[0].entries[2]).toEqual(['headers', ['something', 'right']])
    })
    ...
    test('should not add new entries from multi-line' +
      ...
      const confirmAddEntryButton = component.find('.confirm-add-entry-button')
      await confirmAddEntryButton.trigger('click')
      expect((wrapper.vm as DOMWrapper).rule.sections[0].entries.length).toEqual(2)
    })
  ...
  describe('cancel entry button', () => {
    test('should close adding mode', async () => {
      ...
      expect((wrapper.vm as DOMWrapper).newEntrySectionIndex).toEqual(-1)
    })
  ...
  describe('remove entry button', () => {
    test('should remove entry', async () => {
      ...
      expect((wrapper.vm as DOMWrapper).rule.sections[0].entries.length).toEqual(1)
    })
  ...
  describe('validators', () => {
    ...
    test('should highlight duplicated entries', async () => {
      ...
      await confirmAddEntryButton.trigger('click')
      // check
      expect((wrapper.vm as DOMWrapper).duplicatedEntries.length).toEqual(1)
    })
    ...
    //   // check
    //   expect((wrapper.vm as DOMWrapper).entriesErrors.length).toEqual(1)
    // })
    ...
    test('should clear errors', async () => {
      ...
      // check
      expect((wrapper.vm as DOMWrapper).entriesErrors.length).toEqual(0)
    })
  })
})
